...
class Transpiler : IDisposable
{
    ...
    private Dictionary<string, ushort> CalculateAddressLabels(ILInstruction[] instructions)
    {
        ...
        using var writer = new IL2M6502Writer(ms, logger: _logger)
        {
    ...
    public void Write(Stream stream)
    {
        ...
        int CHR_ROM_SIZE = (int)(chr_rom.Bytes.Length / M6502Writer.CHR_ROM_BLOCK_SIZE);
        ...
        using var writer = new IL2M6502Writer(stream, logger: _logger)
        {
            Instructions = instructions,
            UsedMethods = UsedMethods,
        };
        ...
        {
            using (var tableWriter = new IL2M6502Writer(memoryStream, leaveOpen: true, logger: _logger))
            {
                // Write byte[] table
        ...
        writer.WriteZeroes(M6502Writer.PRG_ROM_BLOCK_SIZE - (PRG_ROM_SIZE % M6502Writer.PRG_ROM_BLOCK_SIZE));

        // Write interrupt vectors
        const int VECTOR_ADDRESSES_SIZE = 6;
        writer.WriteZeroes(M6502Writer.PRG_ROM_BLOCK_SIZE - VECTOR_ADDRESSES_SIZE);
        ...
        // Pad remaining zeros
        int padLength = chr_rom.Bytes.Length % M6502Writer.CHR_ROM_BLOCK_SIZE;
        if (padLength != 0)
        {
    ...
    {
        using var writer = new IL2M6502Writer(new MemoryStream(), logger: _logger)
        {
            UsedMethods = UsedMethods,
            Instructions = instructions,
        };
    ...
    /// With a known value for sizeOfMain
    /// </summary>
    protected virtual void SecondPass(ushort sizeOfMain, IL2M6502Writer writer)
    {
